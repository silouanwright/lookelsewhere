...
| Optional game pause | Implemented; live game recheck required | Sundown's root-owned status provides Steam/Proton and Heroic-launched game activity without duplicating its detectors; accounting pauses without treating gameplay as away time, while focused-window protection remains the standalone fallback |
...
| Keyboard and accessibility | Implemented; host screen-reader export blocked | Named controls and dropdown choices, exposed selection/checked states, transition announcements, deterministic keyboard navigation, native-sized actions, focus-preserving editors, Hardcore lockout, reduced-motion and reduced-transparency behavior, and verified light/dark text contrast. Live keyboard evidence and clean runtime logs are recorded. Quickshell is visible to AT-SPI but currently exports zero child objects, so Orca traversal remains an upstream runtime gap. |
...
| Automated tests and QML review | Implemented | 92 passing model/component tests plus bounded browser, PipeWire, and input checks and an installed-runtime reliability harness covering protected-context fixtures, timer-driven warning/final/break flow, exact configuration restoration, planned breaks, statistics, countdown sequencing, keyboard behavior, enforcement, and sounds |
